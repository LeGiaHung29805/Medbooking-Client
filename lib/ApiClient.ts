...
export const login = async (data: { username: string; password: string }) => {
  ...
  const response = await apiClient.post("/login", {
    email: data.username,   // ✅ CHUẨN BACKEND
...
export const getDoctors = async (
  ...
  // API trả về user array với doctor object bên trong
  if (response.data.success && Array.isArray(response.data.data)) {
    // Transform data: user.doctor -> doctor với user info
  ...
  console.warn('Unexpected doctors response:', response.data);
  return [];
};
