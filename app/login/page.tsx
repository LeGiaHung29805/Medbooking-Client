...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.token) {
        ...
        switch (role) {
          case "QuanTriVien": router.push("/admin"); break;
          case "BacSi": router.push("/Doctor"); break;
          case "NhanVien": router.push("/Staff"); break;
          default: router.push("/dat-lich"); break;
        }
      } else {
  ...
  const handleGetOtp = async () => {
    ...
    try {
      ...
      setModalMessage({ type: 'success', text: "Đã gửi mã OTP vào email của bạn!" });
    } catch (e) {
      setModalMessage({ type: 'error', text: "Lỗi gửi mã. Kiểm tra lại email." });
    } finally {
      setOtpLoading(false);
    }
  };
  ...
  const handleSubmitReset = async () => {
    ...
    if (!forgotEmail || !forgotOtp || !newPassword || !confirmPassword) {
      setModalMessage({ type: 'error', text: "Vui lòng nhập đầy đủ thông tin!" });
      return;
    }
    ...
    <LayoutBook>
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        ...
        <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-md border border-gray-100">
          <h1 className="text-2xl font-bold text-green-700 text-center mb-6">Đăng Nhập</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              ...
              className={`w-full text-white py-2.5 rounded-lg font-bold transition flex justify-center items-center ${isLoading ? "bg-green-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700 shadow-md"
                }`}
            >
          ...
          <p className="text-sm text-center mt-6 text-gray-600">
            Chưa có tài khoản? <a href="/register" className="text-green-600 font-bold hover:underline">Đăng ký ngay</a>
          </p>
        </div>
          ...
          <div className="fixed inset-0 bg-opacity-60 flex items-center justify-center z-50 px-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in-up">
              ...
              <div className="bg-green-600 p-4 flex justify-between items-center text-white">
                ...
                <button onClick={() => setShowForgotModal(false)} className="hover:text-red-200">
                  <FaTimes size={20} />
                </button>
              </div>
              ...
              <div className="p-6">
                <p className="text-sm text-gray-600 mb-4 italic">
                  * Nhập email và nhấn Lấy mã. Sau đó điền mã OTP và mật khẩu mới.
                </p>

                {modalMessage && (
                  <div className={`mb-4 p-3 rounded text-sm font-semibold ${modalMessage.type === 'success'
                  ...
                  <table className="w-full text-sm text-left">
                    <tbody>
                      ...
                      <tr className="border-b border-gray-100">
                        <td className="bg-gray-50 p-3 font-semibold text-gray-700 w-1/3 align-middle">Email</td>
                        <td className="p-2 flex gap-2">
                          ...
                          />
                          <button
                            type="button" onClick={handleGetOtp} disabled={otpLoading}
                      ...
                      <tr className="border-b border-gray-100">
                        <td className="bg-gray-50 p-3 font-semibold text-gray-700 align-middle">Mã OTP</td>
                        <td className="p-2">
                          <input
                      ...
                      <tr className="border-b border-gray-100">
                        <td className="bg-gray-50 p-3 font-semibold text-gray-700 align-middle">Mật khẩu mới</td>
                        <td className="p-2">
                          <input
                      ...
                      <tr>
                        <td className="bg-gray-50 p-3 font-semibold text-gray-700 align-middle">Nhập lại mật khẩu</td>
                        <td className="p-2">
                          <input
